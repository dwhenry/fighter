...
describe Game::Player::Actions do
  ...
  describe '#take_action' do
    context 'pick up inventry items' do
      ...
      it "picks up any inventry items in the current location and places them in the inventry" do
        subject.location.add(moveable_object)
        subject.take_action
      ...
      it "can not picks up non inventry item" do
        subject.load_map(map)
        subject.location.add(stationary_object)
        subject.take_action
      ...
      it 'removes items from the map when they have been picked up' do
        subject.location.add(moveable_object)
        subject.take_action
        subject.location.should_not have_object(Game::Object::PlayerKeyTest)
      end
    end
    ...
    context 'run process on items' do
      ...
      it 'calls process on any objects that have a process method' do
        subject.location.add(process_object)
